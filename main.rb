...
class Project 
  ...
  def show(id)
    @response = get($api_base_url + "admin/projects/#{id}?fields=name,shortName,description,createdBy,team(name,usersCount)")
    @results = JSON.parse(@response.body)
...
if ARGV.length > 0
  if !%w(show update delete create).include?(ARGV[1])
